...
generateBtn.addEventListener("click", () => {
  ...
  const doc = new jsPDF();
  const imgWidth = 180; //180
  const imgHeight = 120; //120
  const margin = parseInt(marginRange.value);

  doc.addImage(frontImage, "JPEG", 15, 15, imgWidth, imgHeight); // 15 15
  doc.addImage(
    backImage,
    "JPEG",
    15,
...
document.getElementById("frontDropZone").addEventListener("click", () => {
  document.getElementById("frontFileInput").click();
});
document.getElementById("backDropZone").addEventListener("click", () => {
